...
(function () {
  ...
  function createMotion() {
    ...
    return {
      ...
      /* The blur behind the panel. A plain fade -- nothing about it should draw the eye, it
         just needs to be there or not be there. */
      backdrop: function (node, show) {
        if (!node) return;
        if (!g) {
          node.classList.toggle("bb-open", show);
          return;
        }
        if (show) node.style.visibility = "visible";
        g.killTweensOf(node);
        g.to(node, {
          autoAlpha: show ? 1 : 0,
          duration: d(show ? 0.3 : 0.22),
          ease: show ? EASE_OUT : "power2.in",
          pointerEvents: show ? "auto" : "none",
        });
      },
  ...
  function init() {
    ...
    function closePanel() {
      ...
      motion.backdrop(backdrop, false);
