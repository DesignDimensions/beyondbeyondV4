...
(() => {
  ...
  // the drawer; everywhere else the drawer's <mini-cart-items> is all there is. Neither
  // element's own renderContents() is used: the drawer's pops it open, and the cart
  // page's looks sections up by DOM id rather than by section id.
  const getSectionsToRender = () => {
    const renderer = [document.querySelector('cart-items'), document.querySelector('mini-cart-items')].find(
  ...
  document.addEventListener('change', (event) => {
    if (!event.target.closest('.quantity__input, quantity-input, cart-remove-button, mini-cart-remove-button')) return;
