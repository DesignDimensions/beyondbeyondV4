...
(function () {
  ...
  function FoundersRoutine(root) {
    ...
    this.panelsWrap = root.querySelector('[data-fr-panels]');
  ...
  FoundersRoutine.prototype.swapPanels = function (from, to) {
    ...
    // old position and nothing left in flow to give the rail a height. Every
    // panel is put back to a clean baseline first — position and card
    // styles cleared on all of them, visibility settled on everyone but the
    // one now incoming — so this swap always starts from a known state no
    // matter what the last one left behind.
    this.panels.forEach(function (panel) {
      if (self.gsap) {
        self.gsap.set(panel, { clearProps: 'position,top,left,right' });
        self.gsap.set(panel.querySelectorAll('.fr__card'), { clearProps: 'all' });
      }
    ...
    var animate = this.fx && outgoing && outgoing !== incoming;
    ...
    // outgoing rail cutting out instantly before the incoming one fades in.
    ...
    var outgoingCards = outgoing.querySelectorAll('.fr__card');
    var incomingCards = incoming.querySelectorAll('.fr__card');

    if (this.panelsWrap) this.panelsWrap.classList.add('is-swapping');

    this.swap = this.gsap.timeline({
      onComplete: function () {
        self.swap = null;
        ...
        if (self.panelsWrap) self.panelsWrap.classList.remove('is-swapping');
      },
    });

    this.swap.to(outgoingCards, { opacity: 0, y: -14, duration: 0.35, ease: 'power2.in' }, 0);
