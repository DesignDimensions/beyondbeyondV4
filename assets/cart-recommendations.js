...
/**
 * Quick-add button on a recommendation card: one variant, quantity of one, straight into
 * the cart with the drawer re-rendered from the same response.
 */
class AddToCart extends HTMLElement {
  ...
  onClickHandler() {
    ...
    // On the cart page there is a line-item table to keep in step, and with the drawer
    // switched off there is nowhere to show the result -- both want a full page load.
    if (document.body.classList.contains('template-cart') || !window.cartSettings?.cartDrawer) {
      Shopify.postLink(routes.cart_add_url, {
        parameters: { id: variantId, quantity: 1 },
      });
      return;
    }
    ...
    fetch(`${routes.cart_add_url}`, { ...fetchConfig('javascript'), body })
      ...
      .then((parsedState) => {
        if (parsedState.status === 422) {
          ...
          publish(PUB_SUB_EVENTS.cartUpdate, { source: 'add-to-cart' });
        }
      })
      .catch((e) => {
        console.error(e);
